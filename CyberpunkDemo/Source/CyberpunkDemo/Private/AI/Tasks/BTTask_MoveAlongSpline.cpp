...
void UBTTask_MoveAlongSpline::TickTask(UBehaviorTreeComponent& OwnerComp, uint8* NodeMemory, float DeltaSeconds)
{
	...
	if (!bStartsFromEnd)
	{
		...
		{
			Speed = FMath::Lerp(Speed, Speed * SpeedMultiplierwhenSlowDown, FMath::GetMappedRangeValueClamped(UE::Math::TVector2<float>(StartToSlowDownDistance, 0.0f), UE::Math::TVector2(0.0f, 1.0f), SplineLength - PositionOnSplineInSpaceUnits));
		}
	...
	else
	{
		...
		{
			Speed = FMath::Lerp(Speed, Speed * SpeedMultiplierwhenSlowDown, FMath::GetMappedRangeValueClamped(UE::Math::TVector2<float>(StartToSlowDownDistance, 0.0f), UE::Math::TVector2(0.0f, 1.0f), PositionOnSplineInSpaceUnits));
		}
	...
	Owner->AddMovementInput((SplineLocation - Owner->GetActorLocation()).GetSafeNormal(), FVector::Distance(Owner->GetActorLocation(), SplineLocation));
